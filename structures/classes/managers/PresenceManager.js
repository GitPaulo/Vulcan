...
class PresenceManager {
    ...
    _updatePresence (callback, interval = this.interval) {
        ...
        this.current = {
            timeout: setInterval((cb = () => {
                const presenceData = callback();

                if (!presenceData) {
                    throw new Error(
                        `Callback passed to 'updatePresence' must return presence data.`
                    );
                }
    ...
    usePrevious () {
        if (!this.previous) {
            throw new Error(
                `No previous presence to switch to!`
            );
        }
    ...
    useTwitch () {
        ...
        this._updatePresence(async () => {
            ...
            if (code === 200) {
                ...
                logger.error(
                    `Was unable to set presence. (Bad repsonse code: ${code}`
                );
            }

...
module.exports = (vulcan, folder) => {
  ...
  if (!fs.existsSync(cmdFolderPath)) {
    throw new Error(`Command folder path '${cmdFolderPath}' does not exist!`);
  }

  // Check if folder is in the correct place
  if (path.dirname(cmdFolderPath) !== cmdsFolderPath) {
    throw new Error(`Folder path '${cmdFolderPath}' must be direct child of '${cmdsFolderPath}'.`);
  }

  // Check if $packages folder exists
  if (!fs.existsSync(cmdsPackagesPath)) {
    throw new Error(`The '${packagesFolderName}' folder must exist.`);
  }
  ...
  if (!cmdClassName) {
    throw new Error(`Invalid command class name: '${folder}'.\n` + `Likely bad folder structure?`);
  }
  ...
  if (!fs.existsSync(cmdsDefinitionsPath)) {
    throw new Error(`Could not find a command definition file! Has to be named: '${cmdsDefinitionFile}'.`);
  }
  ...
  if (
    ...
    throw new Error(`Failed parsing commands definitions file '${cmdsDefinitionFile}' from '${cmdsDefinitionsPath}'.`);
  }
  ...
  for (let fileName of cmdFiles) {
    ...
    try {
      ...
      if (cmdID.hasUpperCase()) {
        throw new Error(`Command files must be lowercase.`);
      }
      ...
      if (!cmdDefinition) {
        throw new Error(`Command definition not found for: '${cmdID}'\n` + `Path: ${fullPath}`);
      }
      ...
      if (fs.existsSync(cmdPackagePath)) {
        fs.readdirSync(cmdPackagePath).forEach(file => {
          ...
          // Log
          logger.logTimeEnd(`Package ${file}`, `Loaded (package) file '${file}' for command '${cmdID}'.`);
        });
      }
      ...
      if (cmd.load) {
        if (cmd.load.constructor.name !== 'AsyncFunction') {
          ...
          cmd
            .load(cmdDefinition, packages)
            .then(() => {
              cmd.loaded = true;
            })
            .catch(err => {
              logger.error(`Command '${cmdID}' encountered load issues.\n` + err.stack);
            });
        }
      } else {
        cmd.loaded = true;
      ...
      if (!cmd.execute) {
        throw new Error(`Execute function is undefined in '${cmd.id}'\n` + `Every cmd must implement 'cmd.execute'!`);
      }

      // ! Only if Discord Command
      // Execute function must be asynchronous. (its called that way in message event)
      if (cmdClassName === dirToClass.get('discord') && cmd.execute.constructor.name !== 'AsyncFunction') {
        throw new Error(

...
// Structures
const { DMChannel, TextChannel, APIMessage } = xrequire('discord.js');
...
const handler = async (channel, wrap) => {
  ...
  if (!(channel instanceof DMChannel || channel instanceof TextChannel)) {
    return APIMessage.create(channel, wrap);
  }
  ...
  if (wrap.content && wrap.content.length > api.messageLimits.content) {
  ...
  if (wrap.embed) {
    ...
    if (wrap.embed.title && wrap.embed.title.length > api.messageLimits.embed.title) {
      reasons.push(`Embed limit reached for 'title' >${api.messageLimits.embed.title}`);
    }

    if (wrap.embed.author && wrap.embed.author.name.length > api.messageLimits.embed.author) {
      reasons.push(`Embed limit reached for 'author' >${api.messageLimits.embed.author}`);
    }

    if (wrap.embed.description && wrap.embed.description.length > api.messageLimits.embed.description) {
    ...
    if (wrap.embed.fields) {
      if (wrap.embed.fields.length >= api.messageLimits.embed.fieldLimit) {
        ...
        wrap.embed.fields.forEach(field => {
          if (
            (field.name && field.name.length > api.messageLimits.embed.fields.title) ||
            (field.value && field.value.length > api.messageLimits.embed.fields.value)
          ) {
            counter++;
          }
        });
    ...
    if (wrap.embed.footer && wrap.embed.footer.text.length > api.messageLimits.embed.footer) {
      reasons.push(`Embed limit reached for 'description >${api.messageLimits.embed.description}`);
    }
...
module.exports = async (...args) => {
  ...
  logger.logTimeEnd('FormatHandler@' + clock, `Performance checks on '${__filename}'`);

    ...
    function logger () {
        ...
        let store = function (logType, str) {
            if (!logTypes.get(logType)) {
                throw new Error(
                    `Log type '${logType}' not valid for 'write' function!`
                );
            }
        ...
        return {
            ...
            removeModifier (modID) {
                ...
                if (index < -1) {
                    throw new Error(
                        `Modifier '${modID}' not found in exisiting modifiers!`
                    );
                }
            ...
            setModifiers (mods) {
                if (!Array.isArray(mods)) {
                    throw new Error(
                        `Expected log modifiers in array format!`
                    );
                }
            ...
            plain (text, color = 'white') {
                if (!this.colors.includes(color)) {
                    throw new Error(
                        `Invalid color '${color}' for 'logger.plain'!`
                    );
                }
            ...
            logTimeEnd (id, msg) {
                ...
                this.log(
                    `[Clock: ${
                        leftPadNumber(Math.round(performance.now() - clock, 2), 4)}ms] ${msg && `=> ${msg}` || ``
                    }`
                );

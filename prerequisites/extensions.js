...
fs.readdirSync(extensionsDir).forEach((file) => {
    ...
    if (parts.length !== 2) {
        throw new Error(
            `Invalid extensions.js file name format for: ${file}`
        );
    }

    if (parts[1] !== 'js') {
        throw new Error(
            `Invalid extensions.js file extension for: ${file}`
        );
    }
    ...
    if (!discordClassFunction) {
        throw new Error(
            `Invalid extensions file name class (not part of Discord structures) for: ${file}`
        );
    }

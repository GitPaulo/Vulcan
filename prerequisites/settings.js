...
fs.readdirSync(defaultsPath).forEach((file)  => {
    ...
    if (!fs.existsSync(settingPath)) {
        ...
        logger.log(
            `Copying settings defaults: '${defaultPath}' => '${settingPath}'.`
        );
    ...
    defaultKH.forEach((dPath, index) => {
        ...
        if (!hasCorrectDepth) {
            ...
            if (match === null) {
                throw Error(
                    `Settings string path didnt resolve a common root node.`
                );
            }

            cPath = match[0];
        } else { // Then dPath must be correct
            cPath = dPath;
        }

...
module.exports = async message => {
  // Destructoring
  let client;
  const { user, blacklist } = (client = message.client);
  ...
  try {
    ...
    if (blacklist.get(message.author.id)) {
      return client.emit('commandRestricted', message, `User **blacklisted** from using Vulcan commands.`);
    }
    ...
    if (!message.command) {
      return client.emit('commandInvalid', message);
    }

    // Check if command is disabled
    if (message.command.disabled) {
      return client.emit('commandBlocked', message, `Command \`${message.command.id}\` has been disabled!`);
    }

    // Check if command is loaded
    if (!message.command.loaded) {
      return client.emit('commandBlocked', message, `Command \`${message.command.id}\`has not yet loaded!`);
    }

    // Check Authorisation level of message author
    if (!message.command.allowsUser(message)) {
      return client.emit('commandRestricted', message, `User not authorised to run command \`${message.command.id}\`!`);
    }

    // Check for spam
    if (message.command.isSpamming(message.author)) {
      return client.emit('commandBlocked', message, `\`${message.command.id}\` spam has been detected.`, [
        {
          name: 'Buffer',
    ...
    client.emit('channelError', message.channel, err);
  }
};

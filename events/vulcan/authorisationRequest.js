...
module.exports = async (requester, guild) => {
  if (guild.authorised) {
    return;
  }
  ...
  if (!requester) {
    throw new Error(`Could not resolve requester to a guild member.`);
  }

  // Resolve owners
  const vulcan = guild.client;
  const owners = configuration.hosts.map(id => vulcan.users.resolve(id) || id);
  ...
  owners.forEach(async owner => {
    ...
    const filter = (reaction, user) =>
      (reaction.emoji.name === yes || reaction.emoji.name === no) && user !== vulcan.user;
    ...
    requestMsg.awaitReactions(filter, { max: 1 }).then(collected => {
      // Answer
      ...
      // Log outcome
      logger.log(
        `Authorisation request for guild ${guild.name}(${guild.id}) answered by ${owner.tag}. Answer: ${answer}.`
      );
    });
  });

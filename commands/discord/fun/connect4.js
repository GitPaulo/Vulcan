...
connect4.execute = async message => {
  if (message.parsed.args.length < 1) {
    return message.client.emit('commandMisused', message, `You need to challenge someone. Invalid #args.`);
  }
  ...
  if (challengee === message.client.user) {
    ...
    // Send out challenge and wait for it!
    try {
    ...
    async nextMove() {
      ...
      if (this.players[this.turn - 1].bot) {
        ...
        const filter = (reaction, user) =>
          this.currentPlayer.id === user.id && outerScope.getControlEmojis().includes(reaction.emoji.identifier);
    ...
    async resetControls() {
      ...
      for (let reaction of reactionsThatNeedRemoving) {
        reaction.users.cache
          .filter(user => user !== this.boardMessage.client.user)
          .forEach(user => reaction.users.cache.remove(user.id));
      }
    },

...
media.execute = async message => {
  ...
  if (!scmd) {
    return message.client.emit('commandMisused', message, `This command requires usage of subcommands!`);
  }
  ...
  switch (scmd) {
    ...
    case 'put': {
      if (numArgs < 3) {
        return message.client.emit('commandMisused', `Expected 3 arguments got ${numArgs}.`, message);
      }
    ...
    case 'get': {
      if (numArgs < 2) {
        return message.client.emit('commandMisused', message, `Expected 1 arguments got ${numArgs}.`);
      }
...
media.storeImageFromURL = async (url, fileName) => {
  ...
  try {
    ...
    fs.unlink(this.folderPath, x => logger.warn(x)); // Delete the file async. (But we don't check the result)
